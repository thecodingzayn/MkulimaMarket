...
export default defineEventHandler(async (event) => {
  ...
  console.log('[notify] handler started')
  ...
  console.log('[notify] received body:', JSON.stringify(body))

  const { type, record, old_record } = body

  // Helper: get user email from auth.users via service role
  const getUserEmail = async (userId) => {
    console.log('[notify] fetching email for userId:', userId)
    const { data, error } = await supabase.auth.admin.getUserById(userId)
    if (error) console.error('[notify] getUserEmail error:', error)
    const email = data?.user?.email ?? null
    console.log('[notify] fetched email:', email)
    return email
  }
  ...
  const getProfile = async (userId) => {
    const { data, error } = await supabase
      ...
      .eq('id', userId)
      .single()
    if (error) console.error('[notify] getProfile error:', error)
    console.log('[notify] fetched profile:', data)
    return data
  }

  const send = async (to, subject, html) => {
    if (!to) {
      console.warn('[notify] send() called with no recipient — skipping')
      return
    }
    ...
    else console.log('[notify] email sent successfully, id:', data?.id)
  }

  // ─── LISTING APPROVED / REJECTED ──────────────────────────────────────────
  if (type === 'listing_status_changed') {
    console.log('[notify] handling listing_status_changed')
    const { id, title, status, rejection_reason, user_id } = record
    console.log('[notify] record:', { id, title, status, user_id })
    console.log('[notify] old_record:', old_record)
    ...
    if (status === 'active' && old_record?.status === 'reviewing') {
      console.log('[notify] listing approved — sending email')
      await send(
        email,
        `✅ Your listing is live — ${title}`,
        listingApprovedTemplate({ name, title, listingUrl: `${site}/listings/${id}`, site })
      )
    } else if (status === 'rejected' && old_record?.status !== 'rejected') {
      console.log('[notify] listing rejected — sending email')
      await send(
        email,
      ...
      console.log('[notify] status change did not match any email condition:', { status, old_status: old_record?.status })
    }
  ...
  // ─── NEW MESSAGE ──────────────────────────────────────────────────────────
  if (type === 'new_message') {
    console.log('[notify] handling new_message')
    ...
    if (convError) console.error('[notify] conv fetch error:', convError)
    if (!conv) { console.warn('[notify] conversation not found'); return { ok: true } }

    const recipientId = conv.initiator_id === sender_id
      ? conv.recipient_id
      : conv.initiator_id

    console.log('[notify] message recipient:', recipientId)

    // Check if recipient is actively reading (skip email if so)
    ...
    console.log('[notify] recentReads in last 10 mins:', recentReads)
    if (recentReads > 0) {
      console.log('[notify] recipient is active — skipping email')
      return { ok: true }
    }
  ...
  if (type === 'transport_application_new') {
    console.log('[notify] handling transport_application_new')
    ...
    if (reqError) console.error('[notify] transport request fetch error:', reqError)
    if (!req) { console.warn('[notify] transport request not found'); return { ok: true } }
  ...
  if (type === 'transport_application_updated') {
    console.log('[notify] handling transport_application_updated')
    const { user_id: applicantId, status, request_id } = record

    if (!['accepted', 'rejected'].includes(status)) {
      console.log('[notify] status not accepted/rejected — skipping:', status)
      return { ok: true }
    }
    ...
    if (reqError) console.error('[notify] transport req fetch error:', reqError)
    if (!req) { console.warn('[notify] transport request not found'); return { ok: true } }
  ...
  if (type === 'expiry_reminder') {
    console.log('[notify] handling expiry_reminder')
    ...
    if (expiryError) console.error('[notify] expiry fetch error:', expiryError)
    console.log('[notify] expiring listings found:', expiring?.length ?? 0)
  ...
  if (type === 'inactive_reminder') {
    console.log('[notify] handling inactive_reminder')
    ...
    if (inactiveError) console.error('[notify] inactive users fetch error:', inactiveError)
    console.log('[notify] inactive users found:', inactiveUsers?.length ?? 0)
  ...
  console.warn('[notify] unhandled type:', type)
  return { ok: true }
})

// ─── EMAIL TEMPLATES ──────────────────────────────────────────────────────────

const baseStyle = `font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #f3f4f6; margin: 0; padding: 0;`

const wrapper = (content, site) => `
<!DOCTYPE html>
<html>
...
const listingRejectedTemplate = ({ name, title, reason, editUrl, site }) => wrapper(`
  ...
  <p style="color:#374151;">You can edit your listing and resubmit for review.</p>
...
const newMessageTemplate = ({ name, senderName, listingTitle, messagePreview, conversationUrl, site }) => wrapper(`
  ...
  <p style="color:#6b7280;margin:0 0 20px;">Hi ${name},</p>
  <p style="color:#374151;"><strong>${senderName}</strong> sent you a message about <strong>"${listingTitle}"</strong>.</p>
...
const transportStatusTemplate = ({ name, status, route, cargo, contactPhone, requestUrl, site }) => wrapper(`
  ...
  <p style="color:#6b7280;margin:0 0 20px;">Hi ${name},</p>
  ${status === 'accepted' ? `
    <p style="color:#374151;">Your transport application has been <strong>accepted</strong>.</p>
    <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:16px;margin:20px 0;">
      <p style="margin:0 0 6px;color:#374151;"><strong>Route:</strong> ${route}</p>
      <p style="margin:0 0 6px;color:#374151;"><strong>Cargo:</strong> ${cargo}</p>
    ...
    <p style="color:#374151;">Contact the requester to confirm details and arrange pickup.</p>
  ` : `
    <p style="color:#374151;">Unfortunately your application for <strong>${route}</strong> was not accepted this time.</p>
...
const expiryReminderTemplate = ({ name, title, daysLeft, editUrl, site }) => wrapper(`
  ...
  <p style="color:#374151;">Extend your listing to keep it active and reach more buyers.</p>
...
const inactiveReminderTemplate = ({ name, activeListings, unreadMessages, dashboardUrl, messagesUrl, site }) => wrapper(`
  ...
  <p style="color:#6b7280;margin:0 0 20px;">Hi ${name},</p>
  <p style="color:#374151;">It's been a week since your last visit. Here's what's happening:</p>
  <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:16px;margin:20px 0;">
    <p style="margin:0 0 8px;color:#374151;">📦 <strong>${activeListings}</strong> active listing${activeListings === 1 ? '' : 's'}</p>
    ${unreadMessages > 0
      ? `<p style="margin:0;color:#dc2626;">💬 <strong>${unreadMessages}</strong> unread message${unreadMessages === 1 ? '' : 's'} waiting</p>`
      : `<p style="margin:0;color:#6b7280;">💬 No unread messages</p>`
    }
  </div>
  ${unreadMessages > 0
    ? `<p style="color:#374151;">You have buyers waiting — don't miss a sale!</p>${btn(messagesUrl, 'Read Messages')}`
