  ...
  <div v-if="inline" class="flex flex-col flex-1 overflow-hidden">
    ...
    <div ref="messagesEl" class="flex-1 overflow-y-auto px-5 py-4 space-y-3 bg-gray-50">
      <div v-if="messages.length === 0" class="text-center text-gray-400 text-sm py-16">
        <Icon icon="mdi:hand-wave-outline" class="w-10 h-10 mx-auto mb-2 text-gray-300" />
        ...
        <div v-else class="flex" :class="item.sender_id === user.id ? 'justify-end' : 'justify-start'">
          <div class="max-w-xs px-4 py-2 rounded-2xl text-sm shadow-sm"
            ...
            <div class="flex items-center justify-end gap-1 mt-1">
              ...
              <span v-if="item.sender_id === user.id" class="flex items-center shrink-0">
                <!-- Double grey ticks — delivered, not read -->
                <svg v-if="!item.read" class="w-5 h-3.5" viewBox="0 0 22 11" fill="none">
                  <path d="M1 5.5L5.5 10L15 1" stroke="rgba(255,255,255,0.7)" stroke-width="2"
                    stroke-linecap="round" stroke-linejoin="round"/>
                  <path d="M7 5.5L11.5 10L21 1" stroke="rgba(255,255,255,0.7)" stroke-width="2"
                    stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
                <!-- Double blue ticks — read -->
                <svg v-else class="w-5 h-3.5" viewBox="0 0 22 11" fill="none">
                  <path d="M1 5.5L5.5 10L15 1" stroke="#90CAF9" stroke-width="2"
                    stroke-linecap="round" stroke-linejoin="round"/>
                  <path d="M7 5.5L11.5 10L21 1" stroke="#90CAF9" stroke-width="2"
                    stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
              </span>
            </div>
      ...
      <div v-if="contactViewedAt" class="flex justify-center py-2">
        <div class="bg-blue-50 border border-blue-100 text-blue-600 text-xs px-4 py-2 rounded-full flex items-center gap-2">
          <Icon icon="mdi:phone-outline" class="w-3.5 h-3.5 shrink-0" />
    ...
    <div v-else class="px-4 py-3 border-t bg-gray-50 text-center text-sm text-gray-400 flex items-center justify-center gap-2">
      <Icon icon="mdi:block-helper" class="w-4 h-4" />
      ...
      <div v-if="modelValue" class="fixed inset-0 z-50 flex justify-end">
        ...
        <div class="relative w-full max-w-sm bg-white flex flex-col shadow-2xl h-full">
          ...
          <div class="flex items-center gap-3 px-4 py-3 border-b bg-white shadow-sm">
            <button @click="close"
              class="text-gray-400 hover:text-gray-600 transition mr-1 shrink-0">
