...
<template>
  <div class="max-w-2xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
    ...
    <div class="flex items-center gap-3 mb-5 sm:mb-6">
      <button
        @click="$router.back()"
        class="text-gray-500 hover:text-green-600 transition text-lg"
      >
        ←
      </button>
    ...
    <div
      v-if="error"
      class="bg-red-50 text-red-600 text-xs sm:text-sm px-4 py-3 rounded-lg mb-4"
    >
      {{ error }}
    </div>
    ...
    <div class="bg-white rounded-xl sm:rounded-2xl shadow p-4 sm:p-6 space-y-5">
      ...
      <div>
        ...
        <div
          v-if="currentExpiry"
          class="mb-2 px-4 py-3 rounded-lg text-xs sm:text-sm"
        ...
        >
          ...
          <span v-else>
            ⚠️ This listing expired on {{ currentExpiry }}
          </span>
        </div>
        ...
        <p
          v-if="newExpiryPreview"
          class="text-xs text-green-600 mt-1"
        >
          ✅ New expiry will be
          <strong>{{ newExpiryPreview }}</strong>
        </p>
      </div>
