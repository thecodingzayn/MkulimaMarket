  ...
  <div class="bg-gray-100 min-h-screen py-4 md:py-8">
    <div class="max-w-4xl mx-auto px-3 md:px-4">
      ...
      <div class="flex justify-between items-center mb-4 md:mb-6">
        <h1 class="text-lg md:text-2xl font-bold text-gray-800">🚛 Transport Requests</h1>
        <NuxtLink v-if="user" to="/transport/new"
          class="bg-green-600 hover:bg-green-700 text-white px-3 md:px-5 py-2 md:py-2.5 rounded-xl font-semibold text-xs md:text-sm transition">
          + Post a Request
        </NuxtLink>
      </div>
        ...
        <div v-for="req in requests" :key="req.id"
          ...
          <div class="flex justify-between items-start flex-wrap gap-2">
            <div class="flex-1 min-w-0">
              <p class="font-bold text-gray-800 text-base md:text-lg truncate">
          ...
          <div class="flex justify-between items-center mt-3 md:mt-4 pt-3 border-t border-gray-100 gap-2">
            ...
            <NuxtLink :to="`/transport/${req.id}`"
              class="text-xs md:text-sm font-semibold text-green-600 hover:underline shrink-0">
              View & Apply →
            </NuxtLink>
          </div>
        </div>
